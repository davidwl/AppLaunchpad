...
class AppLaunchpadNavigationManager {
  /**
   * Use these functions for navigation-related features.
   * @name AppLaunchpadNavigation
   */
  constructor() {}
   ...
   */
  updateTopNavigation() {
    window.postMessage({ msg: 'applaunchpad.navigation.update-badge-counters' }, '*');
  }
   ...
   * @example
   * AppLaunchpad.navigation().navigate('/overview')
   * AppLaunchpad.navigation().navigate('users/groups/stakeholders')
   * AppLaunchpad.navigation().navigate('/settings', null, true) // preserve view
   */
   ...
   * @example
   * AppLaunchpad.navigation().openAsModal('projects/pr1/users', {title:'Users', size:'m'});
   */
   ...
   * @since 0.7.6
   * @example
   * AppLaunchpad.navigation().openAsSplitView('projects/pr1/users', {title:'Users', size:'40'});
   */
   ...
   * @since 1.6.0
   * @example
   * AppLaunchpad.navigation().openAsDrawer('projects/pr1/drawer', {header:true, backdrop:true, size:'s'});
   * AppLaunchpad.navigation().openAsDrawer('projects/pr1/drawer', {header:{title:'My drawer component'}, backdrop:true, size:'xs'});
   */
   ...
   * @example
   * AppLaunchpad.navigation().fromContext('project').navigate('/settings')
   */
   ...
   * @returns {linkManager} link manager instance
   * @example
   * AppLaunchpad.navigation().fromClosestContext().navigate('/users/groups/stakeholders')
   */
  fromClosestContext() {
   ...
   * @returns {linkManager} link manager instance
   * @example
   * AppLaunchpad.navigation().withParams({foo: "bar"}).navigate("path")
   *
   * // Can be chained with context setting functions such as:
   * AppLaunchpad.navigation().fromContext("currentTeam").withParams({foo: "bar"}).navigate("path")
   */
   ...
   * @example
   * AppLaunchpad.navigation().goBack({ foo: 'bar' });
   * AppLaunchpad.navigation().goBack(true);
   */
  goBack(goBackValue) {
...
export const navigation = new AppLaunchpadNavigationManager();
