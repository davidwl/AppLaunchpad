// Type definitions for AppLaunchpad Client

export as namespace AppLaunchpadClient;
...
export declare interface LinkManager {
   ...
   * @example
   * AppLaunchpadClient.linkManager().fromClosestContext().navigate('/users/groups/stakeholders')
   */
   ...
   * @returns {linkManager} link manager instance
   * @example
   * AppLaunchpadClient.linkManager().fromContext('project').navigate('/settings')
   */
   ...
   * @since 1.0.1
   * @example
   * AppLaunchpadClient.linkManager().fromParent().navigate('/sibling')
   */
  fromParent: () => this;
   ...
   * @since 1.0.1
   * @example
   * AppLaunchpadClient.linkManager().fromVirtualTreeRoot().navigate('/users/groups/stakeholders')
   */
   ...
   * @example
   * AppLaunchpadClient.linkManager().goBack({ foo: 'bar' });
   * AppLaunchpadClient.linkManager().goBack(true);
   */
  ...
  /**
   * Navigates to the given path in the application hosted by AppLaunchpad. It contains either a full absolute path or a relative path without a leading slash that uses the active route as a base. This is the standard navigation.
   * @memberof linkManager
   * @param {string} path path to be navigated to
   * @param {string} sessionId current AppLaunchpad **sessionId**
   ...
   * @example
   * AppLaunchpadClient.linkManager().navigate('/overview')
   * AppLaunchpadClient.linkManager().navigate('users/groups/stakeholders')
   * AppLaunchpadClient.linkManager().navigate('/settings', null, true) // preserve view
   * AppLaunchpadClient.linkManager().navigate('#?intent=Sales-order?id=13') // intent navigation
   */
   ...
   * @example
   * AppLaunchpadClient.linkManager().navigateToIntent('Sales-settings', {project: 'pr2', user: 'john'})
   * AppLaunchpadClient.linkManager().navigateToIntent('Sales-settings')
   */
  ...
  /**
   * Sends node parameters to the route. The parameters are used by the `navigate` function. Use it optionally in combination with any of the navigation functions and receive it as part of the context object in AppLaunchpad Client.
   * @memberof linkManager
   * @param {Object} nodeParams
   * @returns {linkManager} link manager instance
   * @example
   * AppLaunchpadClient.linkManager().withParams({foo: "bar"}).navigate("path")
   *
   * // Can be chained with context setting functions such as:
   * AppLaunchpadClient.linkManager().fromContext("currentTeam").withParams({foo: "bar"}).navigate("path")
   */
  withParams: (nodeParams: NodeParams) => this;

  /**
   * Sets options to customise route changing behaviour. The parameters are used by the `navigate` function. Use it optionally in combination with any of the navigation functions and receive it as part of the context object in AppLaunchpad Client.
   * @memberof linkManager
   ...
   * @since 1.25.0
   * @example
   * AppLaunchpadClient.linkManager().withOptions(
   ...
   * @example
   * AppLaunchpadClient.linkManager().openAsModal('projects/pr1/users', {title:'Users', size:'m'});
   */
   ...
   * @example
   * AppLaunchpadClient.linkManager().updateModalSettings({title:'AppLaunchpadModal', size:'l'});
   */
   ...
   * @since 0.6.0
   * @example
   * const splitViewHandle = AppLaunchpadClient.linkManager().openAsSplitView('projects/pr1/logs', {title: 'Logs', size: 40, collapsed: true});
   */
   ...
   * @since 1.6.0
   * @example
   * AppLaunchpadClient.linkManager().openAsDrawer('projects/pr1/drawer', {header:true, backdrop:true, size:'s'});
   * AppLaunchpadClient.linkManager().openAsDrawer('projects/pr1/drawer', {header:{title:'My drawer component'}, backdrop:true, size:'xs'});
   */
   ...
   * It prevents AppLaunchpad Core from handling url change after `navigate()`.
   * Used for auto-navigation
   * @since 0.7.7
   * @example
   * AppLaunchpadClient.linkManager().withoutSync().navigate('/projects/xy/foobar');
   * AppLaunchpadClient.linkManager().withoutSync().fromClosestContext().navigate('settings');
   */
  withoutSync: () => this;
   ...
   * @since 1.21.0
   * @example
   * AppLaunchpadClient.linkManager().withoutSync().updateModalPathInternalNavigation('/projects/xy/foobar');
   */
   ...
   * @since 1.19.0
   * @example
   * AppLaunchpadClient.linkManager().preserveQueryParams(true).navigate('/projects/xy/foobar');
   * AppLaunchpadClient.linkManager().preserveQueryParams(false).navigate('/projects/xy/foobar');
   */
  preserveQueryParams: (preserve: boolean) => this;

  /**
   * Gets the applaunchpad route associated with the current micro frontend.
   * @returns {promise} a promise which resolves to a String value specifying the current applaunchpad route
   * @since 1.23.0
   * @example
   * AppLaunchpadClient.linkManager().getCurrentRoute();
   * AppLaunchpadClient.linkManager().fromContext('project').getCurrentRoute();
   * AppLaunchpadClient.linkManager().fromVirtualTreeRoot().getCurrentRoute();
   */
...
export declare interface StorageManager {
   ...
   * @returns {Promise<void>} resolves an empty value when storage operation is over; it will launch an error if storage is no supported, value cannot be stringify or you are using a applaunchpad reserved key
   * @example
   ...
   * @memberof storageManager
   * @param {string} key: used to identify the item
   * @returns {Promise<Object>} resolves item just removed from storage; it will launch an error if storage is no supported or you are using a applaunchpad reserved key
   * @since 1.6.0
   * @example
   ...
   * @since 1.6.0
   * @example
   * AppLaunchpadClient.storageManager().has(key).then((present) => console.log('element is present: '+present));
   */
 ...
 * @memberof Lifecycle
 * @example
 * AppLaunchpadClient.addNodeParams({applaunchpad:'rocks'});
 * AppLaunchpadClient.addNodeParams({applaunchpad:'rocks', false});
 */
...
/**
 * Sends search query parameters to AppLaunchpad core. If it is allowed on node level it will be added to url.
 * @param {Object} searchParams
 * @param {boolean} keepBrowserHistory
 * @memberof Lifecycle
 * @example
 * AppLaunchpadClient.addCoreSearchParams({applaunchpad:'rocks'});
...
/**
 * When the micro frontend is not embedded in the AppLaunchpad Core application and there is no init handshake you can set the target origin that is used in postMessage function calls by AppLaunchpad Client.
 ...
 * The Link Manager allows you to navigate to another route. Use it instead of an internal router to:
  ...
  - Reflect the route.
  - Keep the navigation state in AppLaunchpad.
  * @name linkManager
  */
 ...
 * @memberof Lifecycle
 */
export function isAppLaunchpadClientInitialized(): boolean;
export type isAppLaunchpadClientInitialized = () => boolean;

/**
 * Starts the handshake with AppLaunchpad Core and thereafter resulting in initialization of AppLaunchpad Client. It is always ran by default when importing applaunchpad-client package in your micro frontend. Note that when using 'defer-applaunchpad-init' to defer default initialization you will need to initialize the handshake using this function manually wherever needed.
 * @since 1.12.0
 * @memberof Lifecycle
 */
export function applaunchpadClientInit(): void;
export type applaunchpadClientInit = () => void;
