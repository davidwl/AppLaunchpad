/**
 * Base class for AppLaunchpad web component micro frontends.
 */
export class AppLaunchpadElement extends HTMLElement {
  constructor(options) {
    super();
  ...
  /**
   * Invoked by applaunchpad core if present, internal, don't override.
   * @private
   */
  __postProcess(ctx, applaunchpadClient, module_location_path) {
    ...
    if (this.applaunchpadConfig && this.applaunchpadConfig.styleSources && this.applaunchpadConfig.styleSources.length > 0) {
      let nr_styles = this.applaunchpadConfig.styleSources.length;
      const loadStylesSync = this.applaunchpadConfig.loadStylesSync;
      ...
      this.applaunchpadConfig.styleSources.forEach((element, index) => {
