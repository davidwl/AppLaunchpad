 ...
 * The Link Manager allows you to navigate to another route. Use it instead of an internal router to:
  ...
  * @name linkManager
  */
export class linkManager extends AppLaunchpadClientBase {
  /**
   * @private
   */
  constructor(values) {
  ...
  navigate(path, sessionId, preserveView, modalSettings, splitViewSettings, drawerSettings) {
    ...
    helpers.sendPostMessageToAppLaunchpadCore(navigationOpenMsg);
  }
   ...
   * @since 1.21.0
   * @example
   * AppLaunchpadClient.linkManager().updateModalPathInternalNavigation('microfrontend')
   */
  updateModalPathInternalNavigation(path, modalSettings = {}, addHistoryEntry = false) {
    ...
    const navigationOpenMsg = {
      msg: 'applaunchpad.navigation.updateModalDataPath',
      params: Object.assign(this.options, {
        link: path,
        modal: modalSettings,
  ...
  updateModalSettings(updatedModalSettings = {}) {
    const message = {
      msg: 'applaunchpad.navigation.updateModalSettings',
      updatedModalSettings
    };
    helpers.sendPostMessageToAppLaunchpadCore(message);
  }
  ...
  pathExists(path) {
    ...
    helpers.addEventListener(
      'applaunchpad.navigation.pathExists.answer',
    ...
    const pathExistsMsg = {
      msg: 'applaunchpad.navigation.pathExists',
    ...
    helpers.sendPostMessageToAppLaunchpadCore(pathExistsMsg);
   ...
   * It prevents AppLaunchpad Core from handling the URL change after `navigate()`.
  ...
  getCurrentRoute() {
    ...
    helpers.addEventListener('applaunchpad.navigation.currentRoute.answer', (e, listenerId) => {
      const data = e.data.data;
      const currentRoutePromise = this.getPromise('getCurrentRoute') || {};
    ...
    helpers.sendPostMessageToAppLaunchpadCore({
      msg: 'applaunchpad.navigation.currentRoute',
      data: Object.assign(this.options, {
        id: currentId
      })
    });
