...
class AlgoliaSearcher {
  constructor() {
    ...
    this.index = this.client.initIndex('applaunchpad-project');
    this.searchResult = 8;
  ...
  executeSearch(query) {
    this.index
      ...
      .then(({ hits }) => {
        ...
        AppLaunchpad.globalSearch().showSearchResult([query].concat(hits));
      })
      .catch(err => {
  ...
  transformUrls(hit) {
    hit.url = hit.url.replace('https://docs.applaunchpad-project.io', '');
