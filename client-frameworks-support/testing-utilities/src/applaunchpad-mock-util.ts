export class AppLaunchpadMockUtil {
  ...
  /**
   * Parses the elements added by AppLaunchpadMockModule into the DOM and assigns them to the local this.messages variable
   */
  async parseAppLaunchpadMockedMessages(): Promise<void> {
    try {
      const textElements: string[] = await this.browser.executeScript(() =>
        Array.from(document.getElementById('applaunchpad-debug-vis-cnt').childNodes).map(item => item.textContent)
      );
      ...
      console.debug('Failed to parse applaunchpad mocked messages: ', e);
    }
  }

  /**
   * Mocks the context by sending applaunchpad context messegaes with the desired mocked context as parameter.
  ...
  mockContext = (mockContext: any) => {
    this.browser.executeScript((mockContext: any) => {
      globalThis.postMessage({ msg: 'applaunchpad.get-context', context: mockContext }, '*');
    }, mockContext);
  };

  /**
   * This method serves as a mock for the applaunchpad client pathExists() function.
   * It is used in e2e tests when component being tested utilizes a call to `AppLaunchpadClient.linkManager().pathExists()`
   *
   ...
   * `AppLaunchpadClient.linkManager().pathExists('pathToCheck')`
   *
  ...
  mockPathExists = (path: string, exists: boolean) => {
    this.browser.executeScript(
      (path: string, exists: boolean) => {
        ...
        globalThis.sessionStorage.setItem('applaunchpadMockData', JSON.stringify(pathExistsMockData));
      },
      path,
      exists
    );
  ...
  /**
   * Checks on the printed DOM AppLaunchpad message responses for a modal with given title
  ...
  async modalOpenedWithTitle(title: string): Promise<boolean> {
    ...
    await this.parseAppLaunchpadMockedMessages();
    ...
    const indexFoundModalMessageWTitle = this.messages.findIndex(message => {
      const msgExists =
        message.msg &&
        message.msg === 'applaunchpad.navigation.open' &&
        message.params &&
