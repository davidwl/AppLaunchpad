...
export default class oAuth2ImplicitGrant {
  ...
  getAuthData() {
    return AppLaunchpad.auth().store.getAuthData();
  }
  ...
  login() {
    return new Promise((resolve, reject) => {
      ...
      sessionStorage.setItem('applaunchpad.nonceValue', generatedNonce);
      ...
      )}?storageType=${AppLaunchpad.auth().store.getStorageType()}`;
      settings.oAuthData.state = btoa(encodeURI(window.location.href) + '_applaunchpadNonce=' + generatedNonce);
  ...
  setTokenExpirationAction() {
    ...
    this.expirationCheckIntervalInstance = setInterval(() => {
      ...
      if (tokenExpirationDate - currentDate < expirationCheckInterval) {
        ...
        AppLaunchpad.auth().handleAuthEvent('onAuthExpired', this.settings, undefined, redirectUrl);
      }
