...
# AppLaunchpad Core API

This document outlines the features provided by the AppLaunchpad Core API. It covers these topics:

-   [Configuration](#applaunchpad-config) - functions related to AppLaunchpad configuration
-   [Elements](#elements) - functions related to DOM elements
-   [Authorization](#authorization) - authorization options for AppLaunchpad
-   [Navigation](#applaunchpadnavigation) - functions related to AppLaunchpad navigation
-   [Localization](#applaunchpadi18n) - options related to language, translation, and localization
-   [Custom messages](#custommessages) - custom messages between AppLaunchpad Core and micro frontends
-   [UX](#ux) - functions related to AppLaunchpad's appearance and user interface
-   [Global search](#globalsearch) - functions related to AppLaunchpad's global search
-   [Theming](#theming) - functions related to AppLaunchpad theming capabilties
-   [Feature toggles](#featuretoggles) - functions related to AppLaunchpad's feature toggle mechanism
...
-   `configInput` **[Object](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object)** the AppLaunchpad Core configuration object
...
Returns **[promise](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)** a promise when a custom `storeUserSettings` function in the settings.userSettings section of the AppLaunchpad configuration is implemented. If it is resolved the user settings dialog will be closed. If the promise is rejected the user settings dialog will also closed if the error object has a `closeDialog` property, e.g `reject({ closeDialog: true, message: 'some error' })`. In addition a custom error message can be logged to the browser console.
...
Returns **[HTMLElement](https://developer.mozilla.org/docs/Web/HTML/Element)** the DOM element that wraps the AppLaunchpad content
...
```javascript
AppLaunchpad.auth().store.getStorageKey()
```
...
```javascript
AppLaunchpad.auth().store.getStorageType()
```
...
```javascript
AppLaunchpad.auth().store.getAuthData()
```
...
-   **See: [SplitView Client](https://docs.applaunchpad-project.io/docs/applaunchpad-client-api?section=splitview) for further documentation. These methods from the Client SplitView are also implemented for AppLaunchpad Core: `close`, `collapse`, `expand`, `isCollapsed`, `isExpanded`, `exists`**
...
```javascript
AppLaunchpad.ux().setDocumentTitle('AppLaunchpad');
```
...
```javascript
AppLaunchpad.ux().getDocumentTitle();
```
...
```javascript
AppLaunchpad.globalSearch().closeSearchField();
```
...
```javascript
AppLaunchpad.globalSearch().getSearchString();
```
...
```javascript
AppLaunchpad.globalSearch().setSearchString('searchString');
```
...
```javascript
AppLaunchpad.globalSearch().setSearchInputPlaceholder('HERE input Placeholder');
```
...
```javascript
AppLaunchpad.featureToggles().setFeatureToggle('featureToggleName');
```
...
```javascript
AppLaunchpad.featureToggles().unsetFeatureToggle('featureToggleName');
```
...
```javascript
AppLaunchpad.featureToggles().getActiveFeatureToggleList();
```
